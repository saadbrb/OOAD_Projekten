...
TEST(GildedRoseTest, Day01) {
  ...
  items.push_back(G.createItem("Sulfur's, Hand of Ragnarok", 0, 80));
  items.push_back(G.createItem("Sulfur's, Hand of Ragnarok", -1, 80));
  ...
  EXPECT_EQ("Sulfur's, Hand of Ragnarok", G.items[3]->name);
  EXPECT_EQ(0, G.items[3]->sellIn); // von -1 auf 0 geaendert
  EXPECT_EQ(80, G.items[3]->quality);

  EXPECT_EQ("Sulfur's, Hand of Ragnarok", G.items[4]->name);
  EXPECT_EQ(-1, G.items[4]->sellIn); // von -2 auf -1 geaendert
  EXPECT_EQ(80, G.items[4]->quality);

  EXPECT_EQ("Backstage passes to a ALTAF80ETC concert", G.items[5]->name);
  EXPECT_EQ(14, G.items[5]->sellIn);
  EXPECT_EQ(21, G.items[5]->quality);

  EXPECT_EQ("Backstage passes to a ALTAF80ETC concert", G.items[6]->name);
  EXPECT_EQ(9, G.items[6]->sellIn);
  EXPECT_EQ(50, G.items[6]->quality); // chang 50 in 49 in 50

  EXPECT_EQ("Backstage passes to a ALTAF80ETC concert", G.items[7]->name);
  EXPECT_EQ(4, G.items[7]->sellIn);
  EXPECT_EQ(50, G.items[7]->quality); // change 50 in 49 in 50
}
...
TEST(GildedRoseTest, Day02) {
  ...
  EXPECT_EQ("Sulfur's, Hand of Ragnarok", G.items[3]->name);
  EXPECT_EQ(0, G.items[3]->sellIn); // von -2 auf -1 geänder
  EXPECT_EQ(80, G.items[3]->quality);

  EXPECT_EQ("Sulfur's, Hand of Ragnarok", G.items[4]->name);
  EXPECT_EQ(-1, G.items[4]->sellIn); // von -3 auf -1 geändert
  EXPECT_EQ(80, G.items[4]->quality);
...
TEST(GildedRoseTest, Day20) {
  ...
  EXPECT_EQ(-20, G.items[3]->sellIn);
  EXPECT_EQ(80, G.items[3]->quality);

  EXPECT_EQ("Sulfuras, Hand of Ragnaros", G.items[4]->name);
  EXPECT_EQ(-21, G.items[4]->sellIn);
  EXPECT_EQ(80, G.items[4]->quality);
...
TEST(GildedRoseTest, Day30) {
  ...
  items.push_back(G.createItem("Backstage passes to a ALTAF80ETC concert", 15, 20));
  items.push_back(G.createItem("Backstage passes to a ALTAF80ETC concert", 10, 49));
  items.push_back(G.createItem("Backstage passes to a ALTAF80ETC concert", 4, 49));
  ...
  EXPECT_EQ("Backstage passes to a ALTAF80ETC concert", G.items[7]->name);
  EXPECT_EQ(-26, G.items[7]->sellIn); // changed form -25 to-26
  EXPECT_EQ(0, G.items[7]->quality);
}
...
TEST(NewConjuredItemsTest, Day01) {
  ...
  cout << "\"Conjured\" stock-items degrade in Quality twice as fast as normal stock-items";

  EXPECT_EQ("Conjured Mana Cake", G.items[0]->name);
  EXPECT_EQ(4, G.items[0]->quality); // verboseness overlie 2x so shell
}
